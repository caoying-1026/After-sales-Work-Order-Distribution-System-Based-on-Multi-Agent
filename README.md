...
用户提交售后咨询或工单后，系统首先进行**意图识别**，再由**智能体编排器**按意图将请求自动分流到对应专属智能体（售后账单 / 技术支持 / 通用客服），复杂问题支持多智能体并行协作，无法自动解决时自动**升级转人工**。全程结合 ChromaDB 向量知识库 RAG 检索与多级记忆（Redis 工作记忆 + ChromaDB 长期记忆），并提供 Prometheus 监控与端到端评测，可通过 Docker Compose 一键部署。
